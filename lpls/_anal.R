...
library(readxl)
library(dplyr)
library(lpls)  #install from https://rdrr.io/github/solvsa/lpls/
...
Xdta <- read_excel("x_dataset_sensory_only_sign_Cuong.xls")
Xdta <- data.frame(Xdta[,-1], row.names = Xdta$product)

Ydta <- read_excel("y_dataset_liking_Cuong.xls")
Ydta <- data.frame(Ydta[,-1], row.names = Ydta$id)

tZdta <- read_excel("z_dataset_consumer_Cuong.xls")

tZdta <- data.frame(tZdta[,-1], row.names = tZdta$id)
Zdta <- as.data.frame(t(tZdta))  #Zdta: consumers as rows, attitudes as columns

save(Xdta, Ydta, Zdta, file = "lplsDta.RData")

load("lplsDta.RData")

shortName <- substr(colnames(Ydta),3,5)


# centering and scaling data ----------------------------------------------

Xsc <- as.matrix(scale(Xdta, center = T, scale = T))
Zsc <- as.matrix(scale(Zdta, center = T, scale = T))
Ydct <- Ydta %>% t() %>% scale(center = T, scale = F) %>%
  t() %>% scale(center = T, scale = F) %>% as.matrix()
...
X1 = Xsc; X2 = Ydct; 
X3 = t(Zsc)  #X3: matrix of attitudes as rows, consumers as columns 
